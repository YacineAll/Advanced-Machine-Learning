...
class SequenceDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return x.reshape(-1,1).astype(np.float32), y.astype(np.int64)
...
class Temperature(pl.LightningDataModule):
    ...
    def create_sequences(self, data: np.ndarray, length: int, columns: list, task: int = 0):
        ...
        if task > 0:
            ...
            return arrays[:, :-task], arrays[:, -task:]
...
class LitModel(pl.LightningModule):
    def __init__(self, backbone:RNN, learning_rate:float=1e-3, forcasting:bool=False):
        super().__init__()
